...
// Pick 2 joints to connect when calling this
function distanceCoolness(_start, _end) { 
    // check joint is not empty!
    if (_start != null && _end != null) {
        
        // Idea: an egg. Strecthing and resting inside.
        // Draw ellipse in center where dancer will stand inside at beginning in black
        // Make color of background orange, the intensity decided by the distance between head and foot
        // Make ellipse change from black to orange over time = 1 minute
        // Make ellipse have a heartbeat. 
        
        
        // Make background stuff
        // the 2 joints
        let start = _start;
        let end = _end;
        // distance
        ...
        // make alpha for orange background
        let temp_alpha = map(d, 0, height, 5, 255);
        let _alpha = lerp(p_alpha, temp_alpha, 0.5);
        //console.log(_alpha);
        
        // draw background
        noStroke();
        fill(255,102, 0, _alpha); // warm orange 
        rect(0,0,width,height);
      
        // Draw a line between points (for debug so comment out!) <<<<------------
        stroke(255);
        ...
        //Make ellipse stuff
        // Make variable that goes from 0-255 in 1 minute = 100000 millis
        millisecond = map(millis(), 0, 100000, 0, 255); 
        noStroke();
        
        // Control how fast the heartbeat goes (larger values = faster)
        a+=0.008;
        //console.log(abs(sin(a)));
        
        // control color by using minimum, to make sure values dont become larger than our orange color = fill(255,102, 0); 
        // modulate color by beat to give a sense of heartbeat
        let beat = abs(sin(a)); // will always be between 0 and 1 
        let c = color(min(255,millisecond)* beat, min(102,millisecond/2)* beat, 0);
        fill(c);
        
        // make sure ellipse is always a minimum size of width/4
        let radius = max(width/4,_alpha*4);
